...
AFX_STATIC COleControlSiteOrWnd* AFXAPI _AfxFindSiteOrWnd(CWnd *pWndDlg, CWnd *pWnd)
{
	...
	while (pos != NULL)
	{
		COleControlSiteOrWnd* pSite = pCtrlCont->m_listSitesOrWnds.GetNext(pos);
		if (pSite->m_hWnd == pWnd->m_hWnd)
			return pSite;
	}
	return NULL;
}
...
AFX_STATIC COleControlSiteOrWnd* AFXAPI _AfxFindNextMnem(CWnd* pWndDlg, COleControlSiteOrWnd* pSiteOrWnd, LPMSG lpMsg)
{
	...
	if (pSiteOrWnd && pSiteOrWnd->m_hWnd)
	{
		CWnd *pTemp = _AfxGetChildControl(pWndDlg, CWnd::FromHandle(pSiteOrWnd->m_hWnd));
		if (pTemp)
	...
	while (pWndT != pSiteOrWnd)
	{
		...
		if (COccManager::IsMatchingMnemonic(pWndT, lpMsg))
		{
			if (pWndT->m_hWnd)
			{
				if (::IsWindowEnabled(pWndT->m_hWnd))
					return pWndT;
			}
			else
			{
...
{
	if (pSiteOrWnd->m_hWnd)
	{
		if (!::IsWindowEnabled(pSiteOrWnd->m_hWnd))
			return FALSE;
	}
	else
	{
	...
	if (!pSiteOrWnd)
	{
		...
		if (pCtrlCont)
		{
			...
			while(pos)
			{
				...
				if((pSiteOrWndFocus->m_hWnd && pSiteOrWndFocus->m_hWnd == ::GetFocus()) ||
					(pSiteOrWndFocus->m_pSite && pSiteOrWndFocus->m_pSite == pCtrlCont->m_pSiteFocus))
				{
