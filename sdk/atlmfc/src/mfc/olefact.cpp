...
BOOL COleObjectFactory::UpdateRegistry(BOOL bRegister)
{
	...
	if (bRegister)
	{
		...
		if (bResult == TRUE)
		{
			...
			{
				USES_CONVERSION_EX;
				LPCTSTR pszClassID = OLE2CT_EX(lpszClassID, _ATL_SAFE_ALLOCA_DEF_THRESHOLD);
				...
				int nLen = _sntprintf(szScratch, nBufSize, _T("CLSID\\%s"), pszClassID);
				if (nLen >= 0 && nLen < nBufSize)
				{
