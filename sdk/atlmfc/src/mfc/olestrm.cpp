	...
	if (m_lpStream != NULL)
	{
		...
		if (m_lpStream->Stat(&statstg, 0) == S_OK)
		{
			if (statstg.pwcsName != NULL)
			{
				...
				LPCTSTR szPath = OLE2CT_EX(statstg.pwcsName, _ATL_SAFE_ALLOCA_DEF_THRESHOLD);
				...
				BOOL fSuccess = AfxFullPath(szTemp, szPath);
...
BOOL COleStreamFile::GetStatus(CFileStatus& rStatus) const
{
	...
	if (statstg.pwcsName != NULL)
	{
		...
		lstrcpyn(rStatus.m_szFullName, szPath, _countof(rStatus.m_szFullName));
		CoTaskMemFree(statstg.pwcsName);
	}
	return TRUE;
}
	...
	DWORD nOpenFlags, CFileException* pError)
{
	...
	LPCOLESTR lpszStr = T2COLE_EX(lpszStreamName, _ATL_SAFE_ALLOCA_DEF_THRESHOLD);
	...
	SCODE sc = lpStorage->OpenStream(lpszStr, NULL, nOpenFlags, 0, &m_lpStream);
	...
	DWORD nOpenFlags, CFileException* pError)
{
	...
	SCODE sc = lpStorage->CreateStream(lpszStr, nOpenFlags,	0, 0, &m_lpStream);
